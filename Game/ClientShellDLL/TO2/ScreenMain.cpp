...
LTBOOL CScreenMain::Build()
{
	...
	m_pResume = AddTextItem("Mod Testing", CMD_SINGLE_PLAYER, IDS_HELP_CONTINUE_GAME);
	m_pResume->Enable(LTTRUE);

	CLTGUITextCtrl* pMP = AddTextItem("Online Multiplayer", CMD_MULTI_PLAYER, IDS_HELP_MULTIPLAYER);
	CLTGUITextCtrl* pMPLan = AddTextItem("LAN Multiplayer", CMD_MULTI_PLAYER_LAN, IDS_HELP_MULTIPLAYER_LAN);
	CLTGUITextCtrl* pOptions = AddTextItem("Settings", CMD_OPTIONS, IDS_HELP_OPTIONS);
	CLTGUITextCtrl* pCtrl = AddTextItem("User Setup", CMD_PROFILE, IDS_HELP_PROFILE);

#ifdef _TO2DEMO
	pMP->Enable( LTTRUE );
	pMPLan->Enable( LTTRUE );
#endif
	...
	if (m_pGameType)
	{
		...
		CLTGUITextCtrl *pCtrl = CreateTextItem("Overthrow",	CMD_COOP, 0);
		m_pGameType->AddControl(pCtrl);

		pCtrl = CreateTextItem("Free For All", CMD_DM, 0);
		m_pGameType->AddControl(pCtrl);

		pCtrl = CreateTextItem("Team For All", CMD_TEAM_DM, 0);
		m_pGameType->AddControl(pCtrl);

		pCtrl = CreateTextItem("Apocalypse", CMD_DOOM, 0);
		m_pGameType->AddControl(pCtrl);
	}
