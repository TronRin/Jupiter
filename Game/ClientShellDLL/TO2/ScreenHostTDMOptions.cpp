...
LTBOOL CScreenHostTDMOptions::Build()
{
	...
	CreateTitle("Team For All Settings");

	m_pTeam1 = AddTextItem("Alpha", CMD_TEAM1, IDS_MAX_PLAYERS_HELP);
	m_pTeam2 = AddTextItem("Bravo", CMD_TEAM2, IDS_MAX_PLAYERS_HELP);

	LTIntPt tmp = m_nextPos;

	LTIntPt pos = m_pTeam1->GetBasePos();
	pos.x += kColumn;
	m_pTeam1Name = AddTextItem("Alpha",0,0,pos,LTTRUE);

	pos = m_pTeam2->GetBasePos();
	pos.x += kColumn;
	m_pTeam2Name = AddTextItem("Bravo",0,0,pos,LTTRUE);
